...
// if the user adds the post to add to favourite
...
while ($ListPost = mysqli_fetch_assoc($GetRows))
{
    ...
    while ($ListPosts = mysqli_fetch_assoc($GetImgRows))
    {
        ...
        {
            echo "<div class='col-md-4 py-2' style='margin-top: 20px;'>
            ...
            if (isset($_SESSION['user']))
            {
                ...
                else
                {
                    echo "
                        ...
                        <button name='AddFav' class='btn btn-primary' style='float: right !important;'>favourite</button></form>";
...
?>
          ...
          echo "</div>";
          if($IsAdmin != 0){
            echo "<a href='./post_delete.php?IDNumber=" . $postIDS . "'class='btn btn-danger'>Delete</a>";
        }; echo"
          </div>
</div>";
        }
